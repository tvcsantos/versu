 ...
 * 
 * This type is used during the multi-phase version calculation process and tracks
 * mutable state as cascade effects are calculated. It includes a `needsProcessing`
 * flag to distinguish modules that require updates from those that don't.
 * 
 * The `toVersion` field is initially empty and populated during the final
 * version calculation phase.
 * 
 * @internal
 * 
 * @property module - The module being processed (immutable reference).
 * 
 * @property fromVersion - Original version before any changes (immutable).
 *                        Used as baseline for bump calculations.
 * 
 * @property toVersion - Calculated new version string (mutable).
 *                      Initially empty, populated in applyVersionCalculations().
 * 
 * @property bumpType - Type of version bump to apply (mutable).
 *                     Can be upgraded during cascade processing.
 * 
 * @property reason - Why this module's version is changing (mutable).
 *                   'unchanged' indicates no processing needed.
 * 
 * @property needsProcessing - Whether this module requires a version update (mutable).
 *                            False means module version stays the same.
 * 
 * @see {@link ProcessedModuleChange} - Final immutable representation after processing
 */
