...
export function createModuleSystemFactory(adapterName: string, repoRoot: string): ModuleSystemFactory {
  ...
  switch (adapterName.toLowerCase()) {
    case GRADLE_ID:
      // Create and return Gradle-specific factory
      return new GradleModuleSystemFactory(repoRoot);
    
    // Add future adapter cases here as they are implemented:
    // case MAVEN_ID:
    //   return new MavenModuleSystemFactory(repoRoot);
    // case NPM_ID:
    //   return new NpmModuleSystemFactory(repoRoot);
    // case PYTHON_ID:
    //   return new PythonModuleSystemFactory(repoRoot);
    
    default:
      // Throw descriptive error for unsupported adapters
